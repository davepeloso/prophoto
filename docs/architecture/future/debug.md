# Debug / DevTools (Needs Rebuild)

Status: Partially implemented — needs rebuild to connect to new ingest event contracts
Source: Archived package prophoto-debug
Last reviewed: 2026-04
Purpose:
Ingest pipeline tracing toolkit — records per-method decision traces during image processing, captures config snapshots, and surfaces both via Filament admin pages and artisan CLI commands.

Notes:
- This is NOT currently functional
- The existing implementation depends on legacy ingest event classes that no longer exist after the ingest rebuild
- The architecture (event listeners → trace storage → Filament UI) is sound and worth preserving
- Rebuild task: wire event listeners to new event contracts in prophoto-contracts
- Priority: medium — needed once the new ingest pipeline ships to production
