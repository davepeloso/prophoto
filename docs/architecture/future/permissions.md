...
Source: Archived package prophoto-permissions
Last reviewed: 2026-04
Purpose:
Contextual RBAC extension layer — per-resource permission grants, invitation system, and policy matrix. Note: base RBAC (Spatie integration, roles, studio/org context) is already implemented in prophoto-access.

Notes:
- This is NOT an active package
- Do not implement without revisiting this spec
- Base RBAC already lives in prophoto-access. This spec covers the contextual/per-resource layer and invitation workflow only.
