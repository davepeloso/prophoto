...
class IntelligenceEmbeddingOrchestrator
{
    // Thin-slice note: this orchestrator runs one injected demo embedding generator.
    ...
    public function __construct(
        ...
        protected string $modelName = 'demo-embedding-model',
    ...
    public function handleAssetReady(AssetReadyV1 $event, array $canonicalMetadata = []): int
    {
        ...
        try {
            ...
            if ($result->labels !== []) {
                throw new RuntimeException('Embedding run returned labels; runs must stay generator-scoped.');
            }
            if (count($result->embeddings) !== 1) {
                throw new RuntimeException('Embedding run must return exactly one embedding result.');
            }

            DB::transaction(function () use ($result, $runId): void {
                $this->persistenceService->persistEmbeddings($result);
