{{-- Story 7.4 — Profile Proofing Template --}}
{{-- Centered profile header with circular avatar, DM Sans + DM Serif Display --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body class="bg-gray-950 text-white min-h-screen font-dm-sans" x-data="proofingViewer()">

    {{-- ═══════════════════════════════════════════════════════════════════ --}}
    {{-- Header Section with Circular Avatar --}}
    {{-- ═══════════════════════════════════════════════════════════════════ --}}
    <header class="bg-gray-900 border-b border-gray-800 py-10 px-4 sm:px-6 lg:px-8">
        <div class="max-w-2xl mx-auto text-center">
            {{-- Circular Avatar (first image or placeholder) --}}
            <div class="mb-6 flex justify-center">
                <div class="w-24 h-24 rounded-full overflow-hidden border-2 border-white/20 flex-shrink-0">
            ...
            <h1 class="font-dm-serif text-5xl sm:text-6xl font-light mb-2">{{ $gallery->subject_name }}</h1>
            <p class="text-gray-400 text-sm mb-6">{{ $share->confirmed_email }}</p>
            @if($share->message)
                <p class="text-gray-300 text-sm mb-8 max-w-xl mx-auto">{{ $share->message }}</p>
            @endif
                ...
                <template x-for="(image, index) in images" :key="image.id">
                    ...
                    >
                        ...
                        <div class="absolute top-2 right-2">
                            <span x-show="image.status === 'approved'" class="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-emerald-600/90 rounded-full">✓ Approved</span>
                            <span x-show="image.status === 'approved_pending'" class="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-amber-600/90 rounded-full">⟳ Pending</span>
                            <span x-show="image.status === 'cleared'" class="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-gray-600/90 rounded-full">Cleared</span>
                        </div>
