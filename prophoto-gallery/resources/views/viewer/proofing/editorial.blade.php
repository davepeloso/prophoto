{{-- Story 7.4 — Editorial Proofing Template --}}
{{-- Asymmetric cinematic layout with Cormorant Garamond + Montserrat fonts --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body class="bg-gray-950 text-white min-h-screen font-montserrat" x-data="proofingViewer()">

    {{-- ═══════════════════════════════════════════════════════════════════ --}}
    {{-- Header Section --}}
    {{-- ═══════════════════════════════════════════════════════════════════ --}}
    <header class="bg-gray-900 border-b border-gray-800 py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto">
            <h1 class="font-cormorant text-5xl sm:text-6xl font-light mb-2">{{ $gallery->subject_name }}</h1>
            <p class="text-xs uppercase tracking-widest text-gray-500 mb-6">Photography Submission</p>
            ...
            <div class="mt-6">
                @if(!$isLocked)
                    <button
                        ...
                        :title="submitTooltip"
                        class="px-6 py-2 text-xs uppercase tracking-wider font-medium bg-white text-black hover:bg-gray-200 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                    >
                        <span x-text="submitLabel"></span>
                    </button>
                @else
                    <div class="inline-block px-6 py-2 text-xs uppercase tracking-wider font-medium bg-gray-800 text-gray-400">
                        Submissions Locked
                    </div>
                @endif
            </div>
        ...
        <div class="max-w-7xl mx-auto">
            <div class="space-y-6">
                <template x-for="(image, index) in images" :key="image.id">
                    <div x-show="index === 0 || (index - 1) % 3 === 0" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        {{-- First image in row: full width (or 2/3) --}}
                        <div
                            class="sm:col-span-2"
                            @click="openModal(index)"
                        >
                            <div class="group relative cursor-pointer overflow-hidden rounded-lg bg-gray-800 hover:ring-2 hover:ring-white/30 transition-all aspect-[16/9]">
                                <img
                                ...
                                <div class="absolute top-3 right-3">
                                    <span x-show="images[index]?.status === 'approved'" class="inline-flex items-center px-3 py-1 text-xs font-medium bg-emerald-600/90 rounded-full">✓ Approved</span>
                                    <span x-show="images[index]?.status === 'approved_pending'" class="inline-flex items-center px-3 py-1 text-xs font-medium bg-amber-600/90 rounded-full">⟳ Pending</span>
                                    <span x-show="images[index]?.status === 'cleared'" class="inline-flex items-center px-3 py-1 text-xs font-medium bg-gray-600/90 rounded-full">Cleared</span>
                                </div>

                                {{-- Rating Stars (if enabled) --}}
                                @if($modeConfig['ratings_enabled'])
                                    <div class="absolute bottom-3 left-3" x-show="images[index]?.rating > 0">
                                        <div class="flex gap-0.5">
                                            <template x-for="star in 5" :key="star">
                                                <span class="text-xs" :class="star <= images[index]?.rating ? 'text-yellow-400' : 'text-gray-600'">★</span>
                                            </template>
                                        </div>
                                    </div>
                                @endif
                        ...
                        {{-- Second image in row: 1/3 --}}
                        <template x-if="index + 1 < images.length && (index % 3 === 0)">
                            <div
                                @click="openModal(index + 1)"
                            >
                                <div class="group relative cursor-pointer overflow-hidden rounded-lg bg-gray-800 hover:ring-2 hover:ring-white/30 transition-all aspect-[9/16]">
                                    <img
                                        :src="images[index + 1]?.thumbUrl"
                                    ...
                                    {{-- Status Badge --}}
                                    <div class="absolute top-3 right-3">
                                        <span x-show="images[index + 1]?.status === 'approved'" class="inline-flex items-center px-3 py-1 text-xs font-medium bg-emerald-600/90 rounded-full">✓ Approved</span>
                                        <span x-show="images[index + 1]?.status === 'approved_pending'" class="inline-flex items-center px-3 py-1 text-xs font-medium bg-amber-600/90 rounded-full">⟳ Pending</span>
                                        <span x-show="images[index + 1]?.status === 'cleared'" class="inline-flex items-center px-3 py-1 text-xs font-medium bg-gray-600/90 rounded-full">Cleared</span>
                                    </div>

                                    {{-- Rating Stars (if enabled) --}}
                                    @if($modeConfig['ratings_enabled'])
                                        <div class="absolute bottom-3 left-3" x-show="images[index + 1]?.rating > 0">
                                            <div class="flex gap-0.5">
                                                <template x-for="star in 5" :key="star">
                                                    <span class="text-xs" :class="star <= images[index + 1]?.rating ? 'text-yellow-400' : 'text-gray-600'">★</span>
                                                </template>
                                            </div>
                                        </div>
                                    @endif
