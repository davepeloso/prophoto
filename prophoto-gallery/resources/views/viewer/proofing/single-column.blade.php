{{-- Story 7.4 — Single Column Proofing Template --}}
{{-- Full-width vertical stack with Instrument Serif + Work Sans fonts --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body class="bg-gray-950 text-white min-h-screen font-work-sans" x-data="proofingViewer()">

    {{-- ═══════════════════════════════════════════════════════════════════ --}}
    {{-- Header Section --}}
    {{-- ═══════════════════════════════════════════════════════════════════ --}}
    <header class="bg-gray-900 border-b border-gray-800 py-10 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto text-center">
            <h1 class="font-instrument-serif text-6xl sm:text-7xl font-normal mb-4">{{ $gallery->subject_name }}</h1>
            <p class="text-gray-400 text-sm mb-2">{{ $share->confirmed_email }}</p>
            @if($share->message)
                <p class="text-gray-300 text-sm mb-8 max-w-2xl mx-auto">{{ $share->message }}</p>
            @endif

            {{-- Progress Bar --}}
            <div class="mt-8 space-y-2 max-w-sm mx-auto">
                ...
                <div class="w-full h-1 bg-gray-800 rounded-full overflow-hidden">
                    <div class="h-full bg-emerald-600 transition-all" :style="`width: ${progressPercent}%`"></div>
                </div>
            </div>
            ...
            <div class="mt-8">
                @if(!$isLocked)
                    <button
                        ...
                        :title="submitTooltip"
                        class="px-8 py-2.5 text-sm font-medium bg-emerald-700 hover:bg-emerald-600 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg transition-colors"
                    >
                        <span x-text="submitLabel"></span>
                    </button>
                @else
                    <div class="inline-block px-8 py-2.5 text-sm font-medium bg-gray-800 text-gray-400 rounded-lg">
                        Submissions Locked
                    </div>
                @endif
            </div>
            ...
            <template x-for="(image, index) in images" :key="image.id">
                <div class="space-y-4">
                    ...
                    <div
                        ...
                        class="group relative cursor-pointer overflow-hidden rounded-lg bg-gray-800 hover:ring-2 hover:ring-white/30 transition-all"
                    >
                        <div class="max-h-screen flex items-center justify-center bg-gray-900">
                            <img
                                :src="image.thumbUrl"
                                :alt="image.alt"
                                class="max-w-full max-h-96 object-contain group-hover:scale-105 transition-transform duration-300"
                                loading="lazy"
                            >
                        </div>
                        ...
                        <div class="absolute top-3 right-3">
                            <span x-show="image.status === 'approved'" class="inline-flex items-center px-3 py-1 text-xs font-medium bg-emerald-600/90 rounded-full">✓ Approved</span>
                            <span x-show="image.status === 'approved_pending'" class="inline-flex items-center px-3 py-1 text-xs font-medium bg-amber-600/90 rounded-full">⟳ Pending</span>
                            <span x-show="image.status === 'cleared'" class="inline-flex items-center px-3 py-1 text-xs font-medium bg-gray-600/90 rounded-full">Cleared</span>
                        </div>

                        {{-- Rating Stars (bottom-left, if enabled) --}}
                        @if($modeConfig['ratings_enabled'])
                            <div class="absolute bottom-3 left-3" x-show="image.rating > 0">
                                <div class="flex gap-0.5">
                                    <template x-for="star in 5" :key="star">
                                        <span class="text-sm" :class="star <= image.rating ? 'text-yellow-400' : 'text-gray-600'">★</span>
                                    </template>
                                </div>
                            </div>
                        @endif
                    ...
                    {{-- Image Info / Caption Below --}}
                    <div class="px-2">
                        <h3 class="font-instrument-serif text-lg font-normal text-white" x-text="image.filename"></h3>
                        <div class="mt-2 flex items-center gap-2 text-sm text-gray-400">
                            <span x-text="(index + 1) + ' of ' + images.length"></span>
                        </div>
                    </div>
                </div>
            </template>
