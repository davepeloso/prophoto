{{-- Story 7.4 — Classic Proofing Template --}}
{{-- Balanced grid with inline ratings and image numbers, Libre Baskerville + Source Sans 3 --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body class="bg-gray-950 text-white min-h-screen font-source-sans" x-data="proofingViewer()">
    ...
    <header class="bg-gray-900 border-b border-gray-800 py-10 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto text-center">
            <h1 class="font-baskerville text-5xl sm:text-6xl font-normal mb-4">{{ $gallery->subject_name }}</h1>
            <p class="text-gray-400 mb-2">{{ $share->confirmed_email }}</p>
            @if($share->message)
            ...
            <div class="mt-8">
                @if(!$isLocked)
                    <button
                        ...
                        :title="submitTooltip"
                        class="px-8 py-3 text-sm font-medium bg-emerald-700 hover:bg-emerald-600 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg transition-colors"
                    >
                        <span x-text="submitLabel"></span>
                    </button>
                @else
                    <div class="inline-block px-8 py-3 text-sm font-medium bg-gray-800 text-gray-400 rounded-lg">
                        Submissions Locked
                    </div>
                @endif
            </div>
    ...
    {{-- ═══════════════════════════════════════════════════════════════════ --}}
    {{-- Image Grid with Numbers and Ratings --}}
    {{-- ═══════════════════════════════════════════════════════════════════ --}}
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                <template x-for="(image, index) in images" :key="image.id">
                    ...
                    >
                        ...
                        <div class="absolute bottom-2 left-2 text-lg font-baskerville font-bold text-white/80">
                            <span x-text="index + 1"></span>
                        </div>

                        {{-- Status Badge (top-right) --}}
                        <div class="absolute top-2 right-2">
                            <span x-show="image.status === 'approved'" class="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-emerald-600/90 rounded">✓</span>
                            <span x-show="image.status === 'approved_pending'" class="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-amber-600/90 rounded">⟳</span>
                            <span x-show="image.status === 'cleared'" class="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-gray-600/90 rounded">−</span>
                        </div>

                        {{-- Rating Stars (bottom-right, if enabled) --}}
                        @if($modeConfig['ratings_enabled'])
                            <div class="absolute bottom-2 right-2" x-show="image.rating > 0">
                                <div class="flex gap-0.25">
                                    <template x-for="star in 5" :key="star">
                                        <span class="text-xs leading-none" :class="star <= image.rating ? 'text-yellow-400' : 'text-gray-600'">★</span>
                                    </template>
                                </div>
                            </div>
                        @endif
